 ...
 * Passport service
 *
 * @description :: Passport text templates
 * @module      :: api/services/PassportService
...
PassportService.connect = function (req, query, profile, next){
    ...
    if (!provider) {
        return next(new Error('s.passport.no.authentication.provided'));
    }
    ...
    if (!user.username && !user.email) {
        return next(new Error('s.passport.no.username.no.email'));
    }
...
PassportService.callback = function (req, res, next){
    ...
    if ('local' === provider && undefined !== action) {
        if ('register' === action && !req.user) {
            this.protocols.local.register(req, res, next);
        }
        else if ('connect' === action && req.user) {
            this.protocols.local.connect(req, res, next);
        }
        else if ('disconnect' === action && req.user) {
            this.disconnect(req, res, next);
        }
        else {
            next(new Error('s.passport.invalid.action'));
        }
    }
    else {
        if ('disconnect' === action && req.user) {
            this.disconnect(req, res, next) ;
        }
        else {
...
PassportService.loadStrategies = function (){
    ...
    Object.keys(strategies).forEach(function (key){
        ...
        if ('local' === key) {
            ...
            if ('oauth' === protocol || 'oauth2' === protocol) {
                options.callbackURL = url.resolve(baseUrl, callback);
            }
            else if ('openid' === protocol) {
...
PassportService.disconnect = function (req, res, next){
    ...
    query.user = user.id;
    query['local' === provider ? 'protocol' : 'provider'] = provider;
